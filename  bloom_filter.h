...
class BloomFilter {
public:
   BloomFilter(float false_positive_rate, size_t expected_items, std::string checkpoint_file) {
    num_bits_ = -1 * (expected_items * std::log(false_positive_rate)) / (std::pow(std::log(2), 2));

    num_hashes_ = num_bits_ / expected_items * std::log(2);
    data_ = new uint8_t[num_bytes()]();
    count_ = 0;
    checkpoint_rate_ = 1000;
   ...
   bool insert(T data){
        ...
        if (inserted) count_++;
        if (count_ % checkpoint_rate_ == 0) checkpoint();
   ...
   bool contains(T data) const {
        ...
        for (size_t i = 0; i < num_hashes_; i++) {
            size_t index = (hash_a + i * hash_b) % num_bits_;
            if (!get_bit(index)) return false;
        }
        return true;
   }

   void checkpoint(){
        FILE *f = fopen(checkpoint_file_.c_str(), "wb");   // "w" = write (truncates), "a" = append, "wb" for binary
        if (f == NULL) {
            perror("fopen");
            return;
        }
        fwrite(data_, 1, num_bytes(), f);
        fclose(f);
   }
